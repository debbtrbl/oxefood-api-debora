...
public class ClienteService {
    ...
    public Cliente save(Cliente cliente, Usuario usuarioLogado) {
        ...
        if (!cliente.getFoneCelular().startsWith("(81)")) {
            throw new ClienteException(ClienteException.MSG_PREFIXO_DDD);
        }

        usuarioService.save(cliente.getUsuario()); // cria o usuario do cliente

        for (Perfil perfil : cliente.getUsuario().getRoles()) { // habilita os perfis do usuario
        ...
        Cliente clienteSalvo = repository.save(cliente); // insere no banco de dados
